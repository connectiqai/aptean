...
def summary_report(product_name, input_data):
    ...
    aqua_header_colored = workbook.add_format({
        **font_base,
        'align': 'center',
        'valign': 'vcenter',
        'bg_color': '#E5F1FB',
    ...
    worksheet.write('B4', 'Top 10 LV3 Categories - Potential impact on Case count Reduction', header_wrap_left_colored)
    worksheet.write('C4', input_data['case_reduction_count_top_10_categories'], border_left)
    worksheet.write('D4', input_data['case_reduction_percent_top_10_categories'] / 100.0, border_left_percent)
    worksheet.write('E4', 'LV1 (Top Level Categoty)', header_wrap_left_colored)
    worksheet.write('F4', 'LV2 (Mid Level Category)', header_wrap_left_colored)
    worksheet.write('G4', 'LV3 (Granular Category)', header_wrap_left_colored)

    worksheet.write('B5', '', border_left)
    worksheet.write('C5', '', border_left)
    worksheet.write('D5', '', border_left)
    ...
    i += 1
    # Categorization
    worksheet.write(f'B{i}', 'LV1 High Level  Categories (Top-10)', highlight_yellow)
    ...
    i=91
    worksheet.write(f'B{i}', 'LV3 Granular Categories (Top-10)', highlight_yellow)
    ...
    worksheet.write(f'F{i}', 'Contribution on Total Case Inflow', header_wrap_colored)
    worksheet.write(f'G{i}', 'Impact on SaaS Customers', header_wrap_colored)
    ...
    i += 2
    worksheet.write(f'B{i}', 'Overall Case prevention potential', highlight_yellow)
    ...
    i += 2
    worksheet.write(f'B{i}', 'Accounts Across LV3 Categories', highlight_yellow) 
    worksheet.write(f'C{i}', 'Top 20', border)
    worksheet.write(f'I{i}','Total LV3 Categories ',highlight_yellow)
    worksheet.write(f'J{i}',input_data.get('total_unique_lv3',''),border)
    ...
    i += 2
    worksheet.write(f'B{i}', 'Accounts that are Top Case contributors', highlight_yellow)
    worksheet.write(f'C{i}', 'Top 20', border)
    ...
    worksheet.insert_chart('B67', bar_chart)
...
def report_process(product_name, cases_df, output_file_path):
    try:
        ...
        new_contributor_category = contributor_category.copy()
        new_contributor_category.append("Knowledge Base Candidate")

        contributor_top20_list=[]
        for category in new_contributor_category:
            try:
                cont_dict =  dict()
                cont_df = pd.read_excel(output_file_path, sheet_name=category).fillna("")
                ...
                pass # To handle if Knowledge Base Candidate sheet is not availble.
        ...
        input_data['total_unique_lv3']=total_unique_lv3
        ...
        print(f"report_process - error: {e}")
        traceback.print_exc()
